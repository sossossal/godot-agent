...
class NonLiveValidationShardsTestCase(unittest.TestCase):
    ...
    def test_preview_supports_release_profile_and_slow_threshold(self):
        ...
        self.assertEqual(payload["report_state"], "preview")
        ...
        self.assertEqual(payload["planned_shard_count"], len(shard_ids))
        self.assertEqual(payload["completed_shard_count"], 0)
        self.assertEqual(payload["pending_shard_count"], len(shard_ids))
        self.assertEqual(payload["pending_shards"], shard_ids)
    ...
    def test_single_shard_report_exposes_status_counts(self):
        ...
        try:
            ...
            self.assertEqual(payload["report_state"], "complete")
            self.assertEqual(payload["shard_count"], len(results))
            self.assertEqual(payload["planned_shard_count"], 1)
            self.assertEqual(payload["completed_shard_count"], len(results))
            self.assertEqual(payload["pending_shard_count"], 0)
            self.assertEqual(payload["pending_shards"], [])
            ...
            self.assertIn("- Report state: complete", markdown)
            self.assertIn("- Planned shards: 1", markdown)
            self.assertIn(f"- Completed shards: {payload['completed_shard_count']}", markdown)
            self.assertIn("- Pending shards: 0", markdown)
    ...
    def test_pr_release_gate_preview_maps_stages_to_profiles(self):
        ...
        self.assertEqual(evidence["non_live"]["report_state"], "preview")
        self.assertEqual(evidence["non_live"]["profile"], "release")
        self.assertEqual(evidence["non_live"]["planned_shard_count"], 0)
        self.assertEqual(evidence["non_live"]["completed_shard_count"], 0)
        self.assertEqual(evidence["non_live"]["pending_shard_count"], 0)
        self.assertEqual(evidence["non_live"]["pending_shards"], [])
        ...
        self.assertEqual(evidence["release_live_preflight"]["check_count"], 0)
        self.assertEqual(evidence["release_live_preflight"]["passed_count"], 0)
        self.assertEqual(evidence["release_live_preflight"]["blocking_count"], 0)
        self.assertEqual(evidence["release_live_preflight"]["warning_count"], 0)
        self.assertEqual(evidence["release_live_preflight"]["status_counts"], {"passed": 0, "blocked": 0, "warning": 0})
    ...
    def test_pr_release_gate_workflow_uses_lightweight_pr_preflight(self):
        ...
        self.assertIn("- Non-live report state:", gate_script)
        self.assertIn("- Non-live completed shards:", gate_script)
        self.assertIn("- Non-live pending shards:", gate_script)
    ...
    def test_customer_trial_bundle_preview_runs_doctor_and_customer_gate(self):
        ...
        self.assertEqual(payload["readiness_summary"]["status"], "preview")
        self.assertEqual(payload["readiness_summary"]["readiness_level"], "preview")
        ...
        self.assertEqual(payload["live_preflight_summary"]["status"], "preview")
        self.assertEqual(payload["live_preflight_summary"]["check_count"], 0)
        self.assertEqual(payload["live_preflight_summary"]["passed_count"], 0)
        self.assertEqual(payload["live_preflight_summary"]["blocking_count"], 0)
        self.assertEqual(payload["live_preflight_summary"]["warning_count"], 0)
        self.assertEqual(payload["live_preflight_summary"]["status_counts"], {"passed": 0, "blocked": 0, "warning": 0})
        self.assertEqual(payload["live_preflight_summary"]["blocking_checks"], [])
        self.assertEqual(payload["live_preflight_summary"]["warning_checks"], [])
        self.assertTrue(payload["live_preflight_summary"]["report_path"].endswith("release_live_preflight.json"))
        self.assertIsNone(payload["gate_summary"])
        self.assertIsNone(payload["readiness_summary"]["gate_summary"])
        ...
        self.assertIsNone(payload["readiness_summary"]["non_live_summary"])
    ...
    def test_customer_trial_bundle_collects_recommended_actions_when_blocked(self):
        ...
        try:
            ...
            self.assertIn(f"- Gate planned step ids: {', '.join(payload['gate_summary']['planned_step_ids'])}", markdown)
            self.assertIn(f"- Gate skipped steps: {payload['gate_summary']['skipped_step_count']}", markdown)
            gate_skipped_summary_label = (
                ", ".join(payload["gate_summary"]["skipped_step_ids"])
                if payload["gate_summary"]["skipped_step_ids"]
                else "None"
            )
            ...
            self.assertIn(f"- Gate total seconds: {payload['gate_summary']['total_duration_seconds']}", markdown)
            self.assertIn(
                f"- Gate slowest step: {payload['gate_summary']['slowest_step_id']} ({payload['gate_summary']['slowest_step_seconds']}s)",
                markdown,
            )
            ...
            self.assertEqual(live_summary["blocking_count"], len(live_summary["blocking_checks"]))
            self.assertEqual(live_summary["warning_count"], len(live_summary["warning_checks"]))
            self.assertEqual(live_summary["status_counts"]["passed"], live_summary["passed_count"])
            self.assertEqual(live_summary["status_counts"]["blocked"], live_summary["blocking_count"])
            self.assertEqual(live_summary["status_counts"]["warning"], live_summary["warning_count"])
            ...
            self.assertIn("- Non-live completed shards: 0", markdown)
            self.assertIn("- Non-live pending shards: 0", markdown)
            self.assertIn("- Non-live passed shards: 0", markdown)
            self.assertIn("- Non-live blocked shards: 0", markdown)
            self.assertIn("- Non-live timeout shards: 0", markdown)
            ...
            gate_summary_payload = payload["gate_summary"]
            self.assertEqual(gate_summary_payload["status"], gate_payload["status"])
            self.assertEqual(gate_summary_payload["stage"], gate_payload["stage"])
            self.assertEqual(gate_summary_payload["mode"], gate_payload["mode"])
            self.assertEqual(gate_summary_payload["non_live_profile"], gate_payload["non_live_profile"])
            self.assertEqual(gate_summary_payload["blocked_steps"], gate_payload["blocked_steps"])
            self.assertEqual(gate_summary_payload["warning_steps"], gate_payload["warning_steps"])
            self.assertEqual(gate_summary_payload["planned_step_count"], gate_payload["planned_step_count"])
            self.assertEqual(gate_summary_payload["planned_step_ids"], gate_payload["planned_step_ids"])
            self.assertEqual(gate_summary_payload["skipped_step_count"], gate_payload["skipped_step_count"])
            self.assertEqual(gate_summary_payload["skipped_step_ids"], gate_payload["skipped_step_ids"])
            self.assertEqual(gate_summary_payload["step_count"], gate_payload["step_count"])
            self.assertEqual(gate_summary_payload["step_ids"], gate_payload["step_ids"])
            self.assertEqual(gate_summary_payload["passed_count"], gate_payload["passed_count"])
            self.assertEqual(gate_summary_payload["blocked_count"], gate_payload["blocked_count"])
            self.assertEqual(gate_summary_payload["warning_count"], gate_payload["warning_count"])
            self.assertEqual(gate_summary_payload["status_counts"], gate_payload["status_counts"])
            self.assertEqual(gate_summary_payload["total_duration_seconds"], gate_payload["total_duration_seconds"])
            self.assertEqual(gate_summary_payload["slowest_step_id"], gate_payload["slowest_step_id"])
            self.assertAlmostEqual(gate_summary_payload["slowest_step_seconds"], gate_payload["slowest_step_seconds"])
            self.assertTrue(gate_summary_payload["report_path"].endswith("gate_summary.json"))
            self.assertTrue(gate_summary_payload["markdown_path"].endswith("gate_summary.md"))
            ...
            live_evidence = gate_payload["evidence"]["release_live_preflight"]
            self.assertEqual(live_evidence["blocking_count"], len(live_evidence["blocking_checks"]))
            self.assertEqual(live_evidence["warning_count"], len(live_evidence["warning_checks"]))
            self.assertEqual(live_evidence["check_count"], len(live_evidence["blocking_checks"]) + live_evidence["passed_count"] + live_evidence["warning_count"])
            self.assertEqual(live_evidence["status_counts"]["passed"], live_evidence["passed_count"])
            self.assertEqual(live_evidence["status_counts"]["blocked"], live_evidence["blocking_count"])
            self.assertEqual(live_evidence["status_counts"]["warning"], live_evidence["warning_count"])
            ...
            self.assertIn(f"- Live preflight checks: {live_evidence['check_count']}", gate_summary)
            self.assertIn(f"- Live preflight blocking count: {live_evidence['blocking_count']}", gate_summary)
            self.assertIn(f"- Live preflight warning count: {live_evidence['warning_count']}", gate_summary)
            ...
            self.assertIsNone(readiness_payload["non_live_summary"])
